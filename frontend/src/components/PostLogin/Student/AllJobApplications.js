...
function AllJobApplications(props) {
  ...
  let [jobApplicationsResp,setjobApplicationsResp] = useState({status: 'loading', jobApplications: null});
  ...
  let handleViewProfile = (e) => {
    e.preventDefault();
    let studentProfileId = e.target.id;
    let profile = <Modal show={true} onHide={handleClose} className="modal-70w">
    ...
    return <Row>
      <Col>
        <Card>
          <Card.Body>
            <Card.Title>
              {jobApplication.studentProfile.firstName} {jobApplication.studentProfile.lastName}
            </Card.Title>
            <Card.Text>Applied on {new Date(jobApplication.createdAt).toLocaleString('en-US', {dateStyle: 'full',})}</Card.Text>
            <Card.Text>
