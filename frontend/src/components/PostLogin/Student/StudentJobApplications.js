...
function StudentJobApplications(props) {
  let [applicationResp,setapplicationResp] = useState({status: 'loading',applications: null, queryParams: {}});
  ...
  let handleOnChange = (e) =>{
    ...
    setapplicationResp({status: 'loading',applications: null, queryParams: {status: status}})
  }
  ...
  let application_tag = applicationResp.applications.map(application =>{
        ...
        <Col>
          <Card>
            <Card.Body>
              ...
              <Card.Text>Applied: {new Date(application.createdAt).toLocaleString('en-US', {dateStyle: 'full'})}</Card.Text>
            </Card.Body>
          </Card>
        </Col>
      </Row>
