...
export const PrivateOwnStudentRoute = ({ component: Component, ...rest }) => (
  ...
  <Route
    {...rest}
    render={(props) => {
      const authentic_user = isLoggedIn() && isStudent() && storedUserInfo().profile.id === parseInt(props.match.params.studentProfileId);
      if (authentic_user) {
        return <Component {...props} />;
      }
      console.log('Not authorized to go this url');
...
export const PrivateOwnCompanyRoute = ({ component: Component, ...rest }) => (
  ...
  <Route
    {...rest}
    render={(props) => {
      const authentic_user = isLoggedIn() && isCompany() && storedUserInfo().profile.id === parseInt(props.match.params.companyProfileId);
      if (authentic_user) {
        return <Component {...props} />;
      }
      console.log('Not authorized to go this url');
